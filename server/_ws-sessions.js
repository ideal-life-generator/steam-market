...
function sessions (settings, callback) {
  ...
  function getCurrentSession (socketSessionId) {
    if (sessions.has(socketSessionId)) {
      return sessions.get(socketSessionId)
    }
    else {
  ...
  wsServer.on("connection", (socket) => {
    ...
    const currentSession = getCurrentSession(socketSessionId)
